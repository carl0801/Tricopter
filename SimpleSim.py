...
try:
    while True:
        ...
        # corregate for position
        if error_x > position_lim:
            target_pitch = -position_correction+kp_position_x*error_x
        elif error_x < -position_lim:
            target_pitch = position_correction+0.03+kp_position_x*error_x
            print(target_pitch)
        else:
            target_pitch = 0.0
        if error_y > position_lim: 
            target_roll = -position_correction+kp_position_y*error_y
        elif error_y < -position_lim:
            target_roll = position_correction+kp_position_y*error_y
        else:
            target_roll = 0.0
